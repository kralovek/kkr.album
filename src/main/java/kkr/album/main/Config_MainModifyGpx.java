...
public class Config_MainModifyGpx {

	private String name;
	private File file;

	public Config_MainModifyGpx(final String[] pArgs)
		...
		if (file != null) {
			try {
				...
				if (!file.getName().toLowerCase().endsWith(".gpx")) {
					throw new ConfigurationException(
							"Parameter '-file' does not contain a GPX file: "
									+ file.getAbsolutePath());
				}
			} catch (Exception ex) {
				throw new ConfigurationException(
						"Bad format of the parameter '-file': "
								+ file.getAbsolutePath());
			}
		}
	}
